  ...
  public render(): JSX.Element {
    ...
    const tabItems = tabList.map((item, index) => {
      ...
      return (
        ...
        >
          <View
            className="at-tabs__item-title"
            style={{
              padding: "8rpx 0",
              fontSize: item.icon ? "24rpx" : "32rpx"
            }}
          >
            {item.icon && (
              <AtIcon value={item.icon} prefixClass="iconfont" size="20" />
            )}
            <View
              className="at-tabs__item-name"
              style={{
                marginTop:
                  item.icon && tabDirection === "vertical" ? "12rpx" : "0rpx"
              }}
            >
              {item.title}
            </View>
