  ...
  public render(): JSX.Element {
    ...
    const tabItems = tabList.map((item, idx) => {
      ...
      return (
        ...
        >
          <View className='at-tabs__item-title' style={{padding: '8rpx 0'}}>
            {item.icon && <AtIcon value={item.icon} prefixClass='icon' size='20'/>}
            <View className='at-tabs__item-name'>{item.title}</View>
            {tabDirection === 'horizontal' && <View className='at-tabs__item-underline'></View>}
          </View>
        </View>
      )
    })
